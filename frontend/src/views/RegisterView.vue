    ...
    <div class="max-w-md w-full space-y-8">
      <div>
        ...
        <p class="mt-2 text-center text-sm text-gray-600">
          Junte-se ao Gerenciador de Tarefas hoje
        </p>
      </div>
      <form class="mt-8 space-y-6 bg-white p-8 rounded-lg shadow-lg" @submit.prevent="handleRegister">
        <div class="rounded-md shadow-sm space-y-4">
          <div>
            <label for="tenant_name" class="block text-sm font-medium text-gray-700 mb-1">Nome da Empresa</label>
            <input
              id="tenant_name"
...
const handleRegister = async () => {
  ...
  try {
    ...
    error.value = err.response?.data?.message || 'Falha no cadastro. Por favor, tente novamente.'
    toast.error(error.value)
  } finally {
    loading.value = false
  }
